...
| 玩家数 | 3/3 + | 2/3 + | 信号判定 |
|--------|--------|--------|----------|
| 3人 | 3/3 → ×1.30 | 2/3 → ×1.15 | 2/3=可靠 |
| 4人 | 4/4 → ×1.40 | 3/4 → ×1.20 | 3/4=可靠 |
...
// 3. 共识权重因子
const CONSENSUS_WEIGHT = { 3: 1.30, 2: 1.15, 1: 1.00 };
...
// 共识权重因子：基于玩家投票（1/3→×1.0, 2/3→×1.15, 3/3→×1.30）
```
